...
export const API_CONFIG = {
  ...
  BASE_URL: process.env.NODE_ENV === 'production' 
    ? 'https://fet7siatbf.execute-api.us-east-1.amazonaws.com/prod'
    : '',
  ...
  ENDPOINTS: {
    ACTIVE_GUESS: '/api/active-guess',
    MAKE_GUESS: '/api/make-guess',
    SCORE: '/api/score',
    BTC_PRICE: '/api/btc-price',
    RESOLVE_GUESS: '/api/resolve-guess',
  },
...
export const getVersionedEndpoint = (endpoint: string, version?: ApiVersion): string => {
  ...
  return endpoint.replace('/api/', `/api/${ver}/`)
} 